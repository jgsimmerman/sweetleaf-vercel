...
import addModification from '../utils/add-total-modification'
...
export default async function submitStripeInfo({ stripeApiSecret, body, verbose }) {
	...
	try {
		...
		if (err.code === `out_of_inventory`) {
			...
			res.messages.error.push(`Sorry! An item in your order exceeds product inventory. Please lower the quantity or remove this product from your cart.`)
		}
